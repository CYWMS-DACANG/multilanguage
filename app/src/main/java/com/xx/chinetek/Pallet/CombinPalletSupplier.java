...
public class CombinPalletSupplier extends BaseActivity {
    ...
    private  boolean lsvPalletDetailonLongClick(AdapterView<?> parent, View view, final int position, long id){
        if(id>=0) {
            ...
            new AlertDialog.Builder(context).setTitle(context.getString(R.string.hint)).setCancelable(false).setIcon(android.R.drawable.ic_dialog_info).setMessage(context.getString(R.string.confirm_delete_material_data_barcode)+barcode)
    ...
    void AnalysisSaveT_PalletDetailADF(String result){
        try {
            ...
            if(returnMsgModel.getMessage().trim().equals(getString(R.string.print_pallet_labe_failed))) {
                MessageBox.Show(context, R.string.pallet_success);
                InitFrm();
            }else{
                MessageBox.Show(context, returnMsgModel.getMessage());
            }
