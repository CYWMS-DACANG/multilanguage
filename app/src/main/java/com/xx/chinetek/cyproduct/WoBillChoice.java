...
public class WoBillChoice extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener{
    ...
    private void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        try {
            ...
            if(BaseApplication.toolBarTitle.Title.equals(context.getString(R.string.Product_manage_subtitle))) {
                intent.setClass(context, ProductManageAdd.class);
                closeActiviry();
            }else if(BaseApplication.toolBarTitle.Title.equals(context.getString(R.string.LineStockInReturnBillChoice))){
                intent.setClass(context, LineStockInReturn.class);
            }else if(BaseApplication.toolBarTitle.Title.equals(context.getString(R.string.LineStockOutMaterial))){
...
//            params.put("No", GsonUtil.parseModelToJson(Fileter));
    ...
    void AnalysisGetT_RESULT_GetT_SyncWoADFJson(String result){
        try {
            ...
            if (returnMsgModel.getHeaderStatus().equals("S")) {
                MessageBox.Show(context, R.string.synchronization_succeeded);
                GetWoinfoModel();
            } else {
                MessageBox.Show(context, returnMsgModel.getMessage());
            }
