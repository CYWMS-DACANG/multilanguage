...
//                    }
                    ...
                    if(BarCodeInfosymh.size()!=0) {
                        ...
                        if(!WgFlag)
                        {
                            MessageBox.Show(context, R.string.not_scan_location);
                            return;
                        }
...
//                }
            ...
            if(R.id.butTui==view.getId()){
                new AlertDialog.Builder(context) .setCancelable(false).setTitle(context.getString(R.string.hint)).setIcon(android.R.drawable.ic_dialog_info).setMessage(context.getString(R.string.confirm_order_deduct_material))
                            .setPositiveButton(context.getString(R.string.config), new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    RequestHandler.addRequestWithDialog(Request.Method.POST, TAG_Get_GetKouLiaoRecord, getString(R.string.get_status), context, mHandler, RESULT_Get_GetKouLiaoRecord, null, URLModel.GetURL().GetKouLiaoRecord, params, null);
                                }
    ...
    void AnalysisGetT_GetKouLiaoRecordAD(String result) {
        try {
            ...
            if (returnMsgModel.getHeaderStatus().equals("S")) {
                new AlertDialog.Builder(context) .setCancelable(false).setTitle(context.getString(R.string.hint)).setIcon(android.R.drawable.ic_dialog_info).setMessage(context.getString(R.string.confirm_material_repeat_deduct))
                        .setPositiveButton(context.getString(R.string.config), new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                // TODO 自动生成的方法
    ...
    void AnalysisGetT_SerialNoByPalletAD(String result){
        try {
            ...
            if (returnMsgModel.getHeaderStatus().equals("S")) {
                ...
                if (!flag){
                    MessageBox.Show(context, R.string.AnalysisGetT_SerialNoByPalletAD);
                    CommonUtil.setEditFocus(edtLineStockInScanBarcode);
                    return;
                }
...
//                    new AlertDialog.Builder(context) .setCancelable(false).setTitle(context.getString(R.string.hint)).setIcon(android.R.drawable.ic_dialog_info).setMessage(context.getString(R.string.confirm_delete_barcode))
//                            .setPositiveButton(context.getString(R.string.config), new DialogInterface.OnClickListener() {
//                                @Override
//                                public void onClick(DialogInterface dialog, int which) {
//                                    // TODO 自动生成的方法
...
//                }
//                if (!flag){
//                    MessageBox.Show(context, context.getString(R.string.AnalysisGetT_SerialNoByPalletAD));
        ...
        try {
            ...
            if (returnMsgModel.getHeaderStatus().equals("S")) {
                ...
                if(String.valueOf(BaseApplication.userInfo.getWarehouseID()).equals(String.valueOf(LWarehouseID))){
                    ...
                    MessageBox.Show(context,R.string.AnalysisetT_GetAreaModelADFJson);
                    editYArea.setText("");
                    CommonUtil.setEditFocus(editYArea);
                }
            } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        try{
            if (item.getItemId() == R.id.action_filter) {
                ...
                //判断是否是散装物料
                if(womodel.getStrVoucherType().equals(context.getString(R.string.bulk_material))){
                    final Map<String, String> params = new HashMap<String, String>();
                    String UserJson = GsonUtil.parseModelToJson(BaseApplication.userInfo);
                    params.put("UserJson", UserJson);
                    params.put("BatchNo", womodel.getWo_Batch().toString());
