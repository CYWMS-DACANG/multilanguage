...
//                    {EditW.setText(message1.contains("ST,GS")?meg[2].trim():"");}
            ...
            MessageBox.Show(context,context.getString(R.string.E_balance_exception));
        }
...
//                    txtMaName.getText().toString().isEmpty()||
            ...
            if (!CommonUtil.isFloat(editNum.getText().toString())){
                MessageBox.Show(context, R.string.qty_format_error);
                return;
            }
            printlabel();
        }catch(Exception ex){
...
//            }
    ...
    void AnalysisGetT_RESULT_Print_OutlabelADFJson(String result){
        try {
            ...
            if (returnMsgModel.getHeaderStatus().equals("S")) {
                MessageBox.Show(context, R.string.return_label_print_success);
            } else {
                MessageBox.Show(context, returnMsgModel.getMessage());
            }
        }catch (Exception ex){
