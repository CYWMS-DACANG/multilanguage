...
public class AdjustCP extends BaseActivity {
    ...
    TextView txtStatus;
    @ViewInject(R.id.txt_strstatus)
    TextView txtstrstatus;
    ...
    void InitFrm(StockInfo_Model barCodeInfo){
        try {
            if (barCodeInfo != null) {
                ...
                String strstatus="";
                switch(barCodeInfo.getStatus()){
                    case 1:strstatus=getString(R.string.test_sample);break;
                    case 2:strstatus=getString(R.string.send_inspection);break;
                    case 3:strstatus=getString(R.string.qualified);break;
                    case 4:strstatus=getString(R.string.unqualified);break;
                }
                txtstrstatus.setText(strstatus);
            }
        }catch (Exception ex){
            MessageBox.Show(context,ex.getMessage());
            CommonUtil.setEditFocus(edtLineStockInScanBarcode);
